...
const BigTextForm = props => {
  ...
  const textCondition = txt => {
    ...
    if (text.length > 0) return Condition(txt);
    return true;
  };
  ...
  const containerStyle = [
    styles.container,
    props.shadow ? Shadow : {},
    textCondition(text) ? {} : styles.wrong,
  ];
  const wrapStyle = [!textCondition(text) ? styles.wrap : {}, props.style];
  return (
    <View style={wrapStyle}>
      ...
      {textCondition(text) ? null : (
        <Text style={[textStyles.helper, styles.error]}>{props.errorMsg}</Text>
      )}
    </View>
  );
};
