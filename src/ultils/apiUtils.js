...
const filterStringtify = filter => {
  const res = '';
  if (filter) res = 'orderBy="' + filter.key + '"&';
  else return null;
  if (filter.type === 'equal') res += `equalTo=${filter.data}`;
  else if (filter.type === 'start') res += `startAt=${filter.data}`;
  else if (filter.type === 'end') res += `endAt=${filter.data}`;
  else return null;
  res += '&';
  return String(res);
};
...
const APIUtils = {
  get: async (url, filter, headers) => {
    ...
    const token = await getFirebaseToken();
    const rsp = axiosClient
      .get(`${url}.json?${flt ? flt : ''}auth=${token}`, {
        headers: {
          'Content-Type': 'application/json',
          Accept: 'text/plain',
          ...headers,
  ...
  patch: async (url, data, headers) => {
    const token = await getFirebaseToken();
    const rsp = axiosClient
      .patch(`${url}.json?auth=${token}`, data, {
        timeout: REQUEST_TIMEOUT,
        headers: {
          'Content-Type': 'application/json',
          ...headers,
